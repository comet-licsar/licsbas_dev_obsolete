...
def count_epoch_occurrences_from_ifg_list(ifg_list):
    ## Count the number of occurrences of each epoch from a list of ifgs:
    primarylist = []
    secondarylist = []
...
def main(argv=None):
    ...
    ## calculate percentage of ifgs associated with each epoch being shortlisted as bad by middle column slope
    shortlist_epochs, shortlist_bad_counts = count_epoch_occurrences_from_ifg_list(coreg_error_ifg)
    all_epochs, all_epoch_counts = count_epoch_occurrences_from_ifg_list(ifgdates)
    likely_bad_epoch = []
    less_likely_bad_epoch = []
    bad_percentage_list = []

    for shortlist_epoch, index in enumerate(shortlist_epochs):
        print(shortlist_epoch)
        bad_count = shortlist_bad_counts[index]
        all_count = all_epoch_counts[all_epochs.index(shortlist_epoch)]
        bad_percentage = bad_count / all_count
        bad_percentage_list.append(bad_percentage)
        if bad_percentage > 0.5:
            likely_bad_epoch.append(shortlist_epoch)
        else:
            less_likely_bad_epoch.append(shortlist_epoch)
    ...
    bad_imfile = os.path.join(infodir, '11candidate_coreg_error_image.txt')
    with open(bad_imfile, 'w') as f:
        for i in bad_imdates:
            print('{}'.format(i), file=f)

    ### Output list of removed image
    bad_imfile = os.path.join(infodir, '11high_probability_coreg_error_image.txt')
    with open(bad_imfile, 'w') as f:
        for i in bad_imdates:
            print('{}'.format(i), file=f)
