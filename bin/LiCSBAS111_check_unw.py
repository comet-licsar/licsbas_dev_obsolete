...
def main(argv=None):
    ...
    ifg_rasdir = os.path.join(tsadir, '111ifg_ras')
    bad_ifg_rasdir = os.path.join(tsadir, '111bad_ifg_ras')
    ...
    lattitude_resolution = int(io_lib.get_param_par(dempar, 'post_lat'))
    ...
    for ifgix, ifgd in enumerate(ifgdates):
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(middle_column_latitudes[non_nan_mask],
                                                                       middle_column[non_nan_mask])
    ...
    coreg_error_epochs, counts = np.unique((primarylist + secondarylist).sort(), return_counts=True)
    ...
    for epoch in coreg_error_epochs:
        ifg_containing_coreg_error_epochs += epoch in ifgdates[:8]
        ifg_containing_coreg_error_epochs += epoch in ifgdates[-8:]

    ## threshold the expanded list with criteria slope_ifg > 20
    ifg_containing_coreg_error_epochs = ifg_containing_coreg_error_epochs[slope_ifg > 20]
    ...
    for i, ifgd in enumerate(ifgdates):
        ...
        if rate_cov[i] < unw_cov_thre or coh_avg_ifg[i] < coh_thre or \
           np.isnan(rate_cov[i]) or np.isnan(coh_avg_ifg[i]) or ifg_containing_coreg_error_epochs[i] > 1:
        ...
        print('{0}  {1:6.1f}  {2:3}   {3:5.3f}   {4:5.3f} {5} {6:5.3f} {7:5.3f}'.format(ifgd, bperp_ifg, dt_ifg, rate_cov[i],  coh_avg_ifg[i], rm_flag, slope_ifg[i], r_square_ifg[i]), file=fstats)
    ...
    bad_ifgfile = os.path.join(infodir, '111bad_ifg.txt')
    ...
    bad_imfile = os.path.join(infodir, '111removed_image.txt')
    ...
    #%% Plot network
    pngfile = os.path.join(netdir, 'network111_all.png')
    plot_lib.plot_network(ifgdates, bperp, [], pngfile)

    pngfile = os.path.join(netdir, 'network111.png')
    plot_lib.plot_network(ifgdates, bperp, bad_ifgdates, pngfile)

    pngfile = os.path.join(netdir, 'network111_nobad.png')
    plot_lib.plot_network(ifgdates, bperp, bad_ifgdates, pngfile, plot_bad=False)


    #%% Finish
    print('\nCheck network/*, 111bad_ifg_ras/* and 111ifg_ras/* in TS dir.')
