...
def main(argv=None):
    ...
    incdir = os.path.join(tsadir,'133increment')
    if not os.path.exists(incdir): os.mkdir(incdir)

    resdir = os.path.join(tsadir,'133resid')
    if not os.path.exists(resdir): os.mkdir(resdir)
    restxtfile = os.path.join(infodir,'133resid.txt')

    cumh5file = os.path.join(tsadir,'cum133.h5')
    ...
    #         raise Usage('No 131ref_de-peaked.txt file exists in {}! Re-run LiCSBAS131_residual_threshold.py and LiCSBAS132_3D_correction'.format(infodir))
    # except Usage as err:
    #     print("\nERROR:", file=sys.stderr, end='')
    #     print("  "+str(err.msg), file=sys.stderr)
    #     print("\nFor help, use -h or --help.\n", file=sys.stderr)
    ...
    imfile = os.path.join(infodir, '133used_image.txt')
    ...
    ### Save image
    rms_cum_wrt_med_file = os.path.join(infodir, '133rms_cum_wrt_med')
    with open(rms_cum_wrt_med_file, 'w') as f:
        rms_cum_wrt_med.tofile(f)

    pngfile = os.path.join(infodir, '133rms_cum_wrt_med.png')
    ...
    refsfile = os.path.join(infodir, '133ref.txt')
    ...
    if width == width_geo and length == length_geo: ## Geocoded
        ...
        io_lib.make_point_kml(reflat, reflon, os.path.join(infodir, '133ref.kml'))
